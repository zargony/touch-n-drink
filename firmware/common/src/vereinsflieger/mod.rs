...
pub struct Connection<'vf, 'conn, T: TcpConnect + 'conn> {
    ...
    last_response_time: Option<TimeReference>,
}

impl<T: TcpConnect> Connection<'_, '_, T> {
    /// Last time and date gathered from API response
    pub fn last_response_time(&self) -> Option<&TimeReference> {
        self.last_response_time.as_ref()
    }
...
impl<T: TcpConnect> Connection<'_, '_, T> {
    ...
    async fn http_json_get<R: DeserializeOwned>(&mut self, path: &str) -> Result<R, Error> {
        ...
        if let Some(time) = parse_time_from_response(&response) {
            self.last_response_time = Some(TimeReference::now(time));
        }
...
fn parse_time_from_response<C: Read>(response: &Response<'_, '_, C>) -> Option<DateTime<Utc>> {
    response
        .headers()
