...
use crate::time;
use crate::user::{User, UserId, Users};
use crate::util::{self, RectangleExt};
use crate::vereinsflieger::{self, Vereinsflieger};
...
/// User interface frontend resources
...
pub trait Backend {
    ...
    type Rng<'a>: RngCore;
    /// Flash storage type for OTA updates
    type Flash<'a>: Storage;
    /// Network stack type
    type Network<'a>: Network;
}

/// User interface backend resources
pub struct BackendResources<'be, BE: Backend> {
    pub rng: BE::Rng<'be>,
    pub flash: BE::Flash<'be>,
    ...
    >,
    pub vereinsflieger: Vereinsflieger<'be>,
    pub telemetry: Telemetry<'be>,
    pub ota: Ota<
        'be,
        <BE::Network<'be> as Network>::TcpConnect<'be>,
        <BE::Network<'be> as Network>::Dns<'be>,
...
enum Error<FE: Frontend> {
    ...
    #[from]
    Vereinsflieger(vereinsflieger::Error),
    /// OTA error
    #[display("OTA: {_0}")]
    #[from]
    Ota(ota::Error),
...
/// User interface
pub struct UserInterface<UI>(pub UI);
...
async fn check_ota_update<FE: Frontend, BE: Backend>(
    ...
    if let Some(new_version) = backend.ota.check().await.map_err(Error::Ota)? {
        show_please_wait(frontend, common::PleaseWait::UpdatingFirmware).await?;

        // Download and apply OTA update
        backend
            .ota
            .update(&new_version, &mut backend.flash)
            .await
            .map_err(Error::Ota)?;
...
async fn submit_telemetry<FE: Frontend, BE: Backend>(
    ...
    let _ = backend.telemetry.flush(&mut backend.http).await;
